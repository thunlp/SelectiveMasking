...
def main():
    ...
    if args.do_train:
        ...
        train_data = indexed_dataset.IndexedDataset(
            args.data_dir, fix_lua_indexing=True)
        if args.local_rank == -1:
            train_sampler = RandomSampler(train_data)
        else:
            train_sampler = DistributedSampler(train_data)
        train_sampler = BatchSampler(
            train_sampler, args.train_batch_size, True)
        ...
        train_iterator = iterators.EpochBatchIterator(
    ...
    param_optimizer = [(n, p) for n, p in param_optimizer if not any(
        nl in n for nl in no_linear)]
    ...
    if args.do_train:
        ...
        fout = open(os.path.join(args.output_dir,
                                 "loss.{}".format(datetime.datetime.now())), 'w')
            ...
            for step, batch in enumerate(tqdm(train_iterator.next_epoch_itr(), desc="Iteration")):
                ...
                if args.fp16:
                    ...
                    loss, original_loss = model(input_ids, segment_ids, input_mask, masked_lm_labels,
                                                input_ent, ent_mask, next_sentence_label, ent_candidate, ent_labels)
                ...
                fout.write("{} {}\n".format(
                    loss.item()*args.gradient_accumulation_steps, original_loss.item()))
                tr_loss += loss.item()
